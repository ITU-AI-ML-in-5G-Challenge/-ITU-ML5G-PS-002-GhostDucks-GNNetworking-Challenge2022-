...
class PklGenerator:
    def __init__(self, data_in, shuffle=False, *_):
        """
        data_in can be a path to pickled dataset or a list of pkl files.
        tf details - string will be received as 'bytes' and list will be received as np.ndarray.
        """
        ...
        # data dir or samples need to be exclusively defined
        if isinstance(data_in, str):
            samples = get_all_pkl_samples(data_in)
        elif isinstance(data_in, np.ndarray):
            samples = data_in
        else:
            raise Exception('PklGenerator misconfiguration')
    ...
    def getitem(self, idx):
        with open(self.samples[idx], 'rb') as handle:
            sample = pickle.load(handle)
        return sample

    def __iter__(self):
        if self.shuffle:
            # to get same order as the original "shuffling"
            # it = list(range(len(self.samples)))
            # random.Random(1234).shuffle(it)
            # (and remove below line)
            it = random.sample(range(self.__len__()), self.__len__())
        else:
            it = range(self.__len__())
...
def create_generator(data_dir=None, shuffle=False, seed=1234, training=False, use_pkl=False, samples=None):
    if data_dir is not None and samples is not None:
        raise Exception('input_fn misconfiguration, both data_dir and samples are not None')
    if use_pkl:
        gen = PklGenerator
        args = [data_dir or samples, shuffle]
    else:
        gen = generator
...
def input_fn(data_dir=None, shuffle=False, seed=1234, training=False, use_pkl=False, samples=None):
    gen, args = create_generator(data_dir, shuffle, seed, training, use_pkl, samples)
    return input_fn_from_gen(gen, args)


def input_fn_from_gen(gen, args):
