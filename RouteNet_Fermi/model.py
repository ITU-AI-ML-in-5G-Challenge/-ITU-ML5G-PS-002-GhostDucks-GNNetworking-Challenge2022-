...
class RouteNet_Fermi(tf.keras.Model):
    def __init__(self, aux_config=None):
    ...
    def call(self, inputs):
        ...
        output = queue_delay + trans_delay
        if self.aux_config is None:
            return output
        else:
            y_pred = tf.RaggedTensor.from_row_lengths(output[:, 0], [tf.shape(output)[0]])
            pp = input_tensor  # F, l, d
            pps = tf.expand_dims(tf.shape(pp), 0)   # 1, 3
            pp = tf.reshape(pp, [-1, self.path_state_dim])      # Fl, d
            out_path_state = tf.RaggedTensor.from_row_lengths(pp, [tf.shape(pp)[0]])
            out_link_state = tf.RaggedTensor.from_row_lengths(link_state, [tf.shape(link_state)[0]])
            out_queue_state = tf.RaggedTensor.from_row_lengths(queue_state, [tf.shape(queue_state)[0]])
            out_cpc = tf.expand_dims(capacity_gather, 0)
            return y_pred, out_path_state, pps, out_link_state, out_queue_state, out_cpc
